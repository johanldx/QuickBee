...
writeLog('/auth/backend/login-process.php', "Connexion", getUserIP());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        $db = new Database();
        $conn = $db->connect();
        $sql = "SELECT email, password, id, active, first_name, last_name FROM user WHERE email = :email";
        $params = [':email' => $email];
    ...
    header('Location: ' . getenv('URL_PATH') . '/auth/login.php?error='.urlencode("Impossible de récupérer les informations."), true, 301);
    exit;
}