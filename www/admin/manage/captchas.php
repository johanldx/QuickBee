...
writeLog('/admin/manage/captcha.php', "Visité la page captcha de l'app", getUserIP(), $_SESSION['logged']);
...
<html lang="fr">
    ...
    <?php displayMessage() ?>
        <div class="d-flex">
            ...
            <main class="main p-3">
                <h1 class="mt-3 mb-3 fs-3 text-start">Captcha<?php if ($result && count($result) > 1) { echo("s"); } ?> </h1>

                <div class="text-end mb-3">
                    <a href="new-captcha.php" class="btn btn-primary">Nouveau captcha</a>
                </div>
                    ...
                    <table class="table mt-3">
                        ...
                        <body>
                            ...
                            <tr>
                                ...
                                <td><?php echo(strlen($row['question']) > 50 ? substr($row['question'], 0, 50) . "..." : $row['question']); ?></th>
                                <td><?php echo(strlen($row['response']) > 50 ? substr($row['response'], 0, 50) . "..." : $row['response']); ?></th>
                                <td><a href="/admin/manage/edit-captcha.php?id=<?php echo($row['id']); ?>" class="btn btn-primary">Modifier</a></th>
                                <td><a href="/admin/backend/delete-captcha-process.php?id=<?php echo($row['id']); ?>" class="btn btn-danger">Supprimer</a></th>
                            </tr>
                            <?php
                                }
                            }
