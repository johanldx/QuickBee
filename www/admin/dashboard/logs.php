...
if (file_exists($filePath)) {
    ...
    $fileContent = "Le fichier sélectionné n'existe pas.";
}

function colorizeLogContent($content) {
    $lines = explode("\n", $content);
    $colorizedLines = array();
    
    foreach ($lines as $line) {
        if (preg_match('/^(.+?) : (.+?) : (.+?) : (.+)$/', $line, $matches)) {
            $date = '<span class="text-warning">' . htmlspecialchars(trim($matches[1])) . '</span>';
            $ipv4 = '<span class="">' . htmlspecialchars(trim($matches[2])) . '</span>';
            $path = '<span class="text-success">' . htmlspecialchars(trim($matches[3])) . '</span>';
            $message = '<span class="">' . htmlspecialchars(trim($matches[4])) . '</span>';

            $colorizedLines[] = $date . ' : ' . $ipv4 . ' : ' . $path . ' : ' . $message;
        } else {
            $colorizedLines[] = htmlspecialchars($line);
        }
    }
    
    return implode("<br>", $colorizedLines);
}

writeLog('/admin/dashboard/logs.php', "Visité la page des logs", getUserIP(), $_SESSION['logged']);
...
<html lang="fr" data-bs-theme="auto">
    ...
    <link rel="stylesheet" href="/static/css/btn-custom.css"></head> 
    ...
    <?php displayMessage() ?>
        <div class="d-flex">
                    ...
                    <li class="sidebar-item">
                        ...
                        <ul id="dashboard" class="sidebar-dropdown list-unstyled collapse" data-bs-parent="#sidebar">
                            ...
                            <li class="sidebar-item item-selected">
                                <a href="" class="sidebar-link text-decoration-none item-selected">Logs</a>
                            </li>
                            <li class="sidebar-item">
            ...
            <main class="main p-3">
                <h1 class="mt-3 mb-3 fs-3 text-start">Journaux de Quickbee</h1>
                ...
                <div class="mt-4">
                    <div class="card bg-light-subtle">
                        <div class="card-body bg-dark border border-light-subtle rounded">
                            <p class="text-light">
                                <?php echo colorizeLogContent($fileContent); ?>
                            </p>
                        </div>
                    </div>
                </div>
